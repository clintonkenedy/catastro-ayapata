                ...
                <div class="container-fluid">
                    <div class="row justify-between items-center q-col-gutter-md">
                        ...
                        <div  class="col-md-auto col-12 " >
                            <q-btn       :rounded="false"  size=""  color="primary" no-caps  unelevated   :to="`/tfunicat/add`" class="full-width" >
                ...
                <div class="row q-col-gutter-x-md">
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && pageBreadCrumb.fieldName">
                                <q-card class="q-pa-md q-mb-sm text-bold nice-shadow-6">
                                    <q-breadcrumbs>
                                    <q-breadcrumbs-el icon="arrow_back" class="text-capitalize" :label="pageBreadCrumb.fieldName" to="/tfunicat"></q-breadcrumbs-el>
                        ...
                        <div >
                            ...
                            :bordered="false"
                            :columns="app.menus.tfunicatlistheader" 
                            :rows="records"
                            ...
                            <template v-slot:body="props">
                                <q-tr :class="{selected: isCurrentRecord(props.row)}" :props="props">
                                    ...
                                    <q-td  key="id_uni_cat" :props="props">
                                        <q-btn padding="xs"   :rounded="false"  color="primary"  no-caps  unelevated   flat :to="`/tfunicat/view/${props.row.id_uni_cat}`">{{ props.row.id_uni_cat }}</q-btn>
                                    </q-td>
                                    <q-td  key="id_lote" :props="props">
                                        {{ props.row.id_lote }}
                                    </q-td>
                                    <q-td  key="id_edificacion" :props="props">
                                        <q-btn v-if="props.row.id_edificacion" @click="app.openPageDialog({ page: 'tfedificaciones/view', url: `/tfedificaciones/view/${props.row.id_edificacion}` , closeBtn: true })" padding="xs" flat color="primary" no-caps >
                                            <q-icon name="visibility" size="xs"  class="q-mr-xs"></q-icon>  Tf Edificaciones
                                        </q-btn>
                                    </q-td>
                                    <q-td  key="codi_entrada" :props="props">
                                        {{ props.row.codi_entrada }}
                                    </q-td>
                                    <q-td  key="codi_piso" :props="props">
                                        {{ props.row.codi_piso }}
                                    </q-td>
                                    <q-td  key="codi_unidad" :props="props">
                                        {{ props.row.codi_unidad }}
                                    </q-td>
                                    <q-td  key="tipo_interior" :props="props">
                                        {{ props.row.tipo_interior }}
                                    </q-td>
                                    <q-td  key="cuc" :props="props">
                                        {{ props.row.cuc }}
                                    </q-td>
                                    <q-td  key="cuc_antecedente" :props="props">
                                        {{ props.row.cuc_antecedente }}
                                    </q-td>
                                    <q-td  key="codi_hoja_catastral" :props="props">
                                        {{ props.row.codi_hoja_catastral }}
                                    </q-td>
                                    <q-td  key="codi_pred_rentas" :props="props">
                                        {{ props.row.codi_pred_rentas }}
                                    </q-td>
                                    <q-td  key="nume_interior" :props="props">
                                        {{ props.row.nume_interior }}
                                    </q-td>
                                    <q-td  key="unid_acum_rentas" :props="props">
                                        {{ props.row.unid_acum_rentas }}
                                    </q-td>
                                    <q-td  key="codi_cont_rentas" :props="props">
                                        {{ props.row.codi_cont_rentas }}
                                    </q-td>
                                            ...
                                            <q-btn icon="menu" padding="xs" round flat color="grey">
                                                <q-menu auto-close transition-show="flip-right"  transition-hide="flip-left" self="center middle" anchor="center middle">
                                                    <q-list dense rounded nav>
                                                        <q-item link clickable v-ripple :to="`/tfunicat/view/${props.row.id_uni_cat}`">
                                                            <q-item-section>
                                                                <q-icon color="primary" size="sm" name="visibility"></q-icon>
                                                            </q-item-section>
                                                            <q-item-section>View</q-item-section>
                                                        </q-item>
                                                        <q-item link clickable v-ripple :to="`/tfunicat/edit/${props.row.id_uni_cat}`">
                                                            <q-item-section>
                                                                <q-icon color="positive" size="sm" name="edit"></q-icon>
                                                            </q-item-section>
                                                            <q-item-section>Edit</q-item-section>
                                                        </q-item>
                                                        <q-item link clickable v-ripple @click="deleteItem(props.row.id_uni_cat)">
                                                            <q-item-section>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName : {
			type : String,
			default : 'tfunicatlist',
		},
		apiPath : {
			type : String,
			default : 'tfunicat/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
