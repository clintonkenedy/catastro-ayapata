                ...
                <div class="container-fluid">
                    <div class="row justify-between items-center q-col-gutter-md">
                        ...
                        <div  class="col-md-auto col-12 " >
                            <q-btn       :rounded="false"  size=""  color="primary" no-caps  unelevated   :to="`/tfpuertas/add`" class="full-width" >
                ...
                <div class="row q-col-gutter-x-md">
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && pageBreadCrumb.fieldName">
                                <q-card class="q-pa-md q-mb-sm text-bold nice-shadow-6">
                                    <q-breadcrumbs>
                                    <q-breadcrumbs-el icon="arrow_back" class="text-capitalize" :label="pageBreadCrumb.fieldName" to="/tfpuertas"></q-breadcrumbs-el>
                        ...
                        <div >
                            ...
                            :bordered="false"
                            :columns="app.menus.tfpuertaslistheader" 
                            :rows="records"
                            ...
                            <template v-slot:body="props">
                                <q-tr :class="{selected: isCurrentRecord(props.row)}" :props="props">
                                    ...
                                    <q-td  key="id_puerta" :props="props">
                                        <q-btn padding="xs"   :rounded="false"  color="primary"  no-caps  unelevated   flat :to="`/tfpuertas/view/${props.row.id_puerta}`">{{ props.row.id_puerta }}</q-btn>
                                    </q-td>
                                    <q-td  key="id_lote" :props="props">
                                        <q-btn v-if="props.row.id_lote" @click="app.openPageDialog({ page: 'tflotes/view', url: `/tflotes/view/${props.row.id_lote}` , closeBtn: true })" padding="xs" flat color="primary" no-caps >
                                            <q-icon name="visibility" size="xs"  class="q-mr-xs"></q-icon>  Tf Lotes
                                        </q-btn>
                                    </q-td>
                                    <q-td  key="codi_puerta" :props="props">
                                        {{ props.row.codi_puerta }}
                                    </q-td>
                                    <q-td  key="tipo_puerta" :props="props">
                                        {{ props.row.tipo_puerta }}
                                    </q-td>
                                    <q-td  key="nume_muni" :props="props">
                                        {{ props.row.nume_muni }}
                                    </q-td>
                                    <q-td  key="cond_nume" :props="props">
                                        {{ props.row.cond_nume }}
                                    </q-td>
                                    <q-td  key="id_via" :props="props">
                                        <q-btn v-if="props.row.id_via" @click="app.openPageDialog({ page: 'tfvias/view', url: `/tfvias/view/${props.row.id_via}` , closeBtn: true })" padding="xs" flat color="primary" no-caps >
                                            <q-icon name="visibility" size="xs"  class="q-mr-xs"></q-icon>  Tf Vias
                                        </q-btn>
                                    </q-td>
                                    <q-td  key="nume_certificacion" :props="props">
                                        {{ props.row.nume_certificacion }}
                                    </q-td>
                                            ...
                                            <q-btn icon="menu" padding="xs" round flat color="grey">
                                                <q-menu auto-close transition-show="flip-right"  transition-hide="flip-left" self="center middle" anchor="center middle">
                                                    <q-list dense rounded nav>
                                                        <q-item link clickable v-ripple :to="`/tfpuertas/view/${props.row.id_puerta}`">
                                                            <q-item-section>
                                                                <q-icon color="primary" size="sm" name="visibility"></q-icon>
                                                            </q-item-section>
                                                            <q-item-section>View</q-item-section>
                                                        </q-item>
                                                        <q-item link clickable v-ripple :to="`/tfpuertas/edit/${props.row.id_puerta}`">
                                                            <q-item-section>
                                                                <q-icon color="positive" size="sm" name="edit"></q-icon>
                                                            </q-item-section>
                                                            <q-item-section>Edit</q-item-section>
                                                        </q-item>
                                                        <q-item link clickable v-ripple @click="deleteItem(props.row.id_puerta)">
                                                            <q-item-section>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName : {
			type : String,
			default : 'tfpuertaslist',
		},
		apiPath : {
			type : String,
			default : 'tfpuertas/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
