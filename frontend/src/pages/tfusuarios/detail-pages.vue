<template>
    <div id="master-detailpage">
        <q-tabs  inline-label v-model="selectedTab" active-color="primary" indicator-color="primary" align="left">
            <q-tab no-caps name="tffichas" icon="extension" label="Tf Usuario Tf Fichas" />
        </q-tabs>
        <q-separator></q-separator>
        <q-tab-panels keep-alive v-model="selectedTab" animated>
            <q-tab-panel class="q-pa-none" name="tffichas">
                <div class="reset-grid">
                    <list-tffichas-page ref="tffichasListPage"  field-name="tf_fichas.id_usuario" :field-value="masterRecord.id_usuario" :show-header="false" :show-breadcrumbs="false" isSubPage>
                        </list-tffichas-page>
                    </div>
                </q-tab-panel>
            </q-tab-panels>
...
const app = useApp();
const store = usePageStore('TFUSUARIOS');
...
function setDetailPageFormData(){
	...
	tffichasStore.setFormData({ id_usuario:record?.id_usuario });
}
